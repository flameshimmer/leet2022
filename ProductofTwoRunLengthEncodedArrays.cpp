...
//nums with many segments of consecutive repeated numbers to be represented by a
//(generally smaller) 2D array encoded. Each encoded[i] = [vali, freqi] describes
//the ith segment of repeated numbers in nums where vali is the value that is
//repeated freqi times.
//For example, nums = [1,1,1,2,2,2,2,2] is represented by the run-length encoded
//array encoded = [[1,3],[2,5]]. Another way to read this is "three 1's followed
//by five 2's".
...
//full arrays nums1 and nums2 respectively. Both nums1 and nums2 have the same
//length. Each encoded1[i] = [vali, freqi] describes the ith segment of nums1,
//and each encoded2[j] = [valj, freqj] describes the jth segment of nums2.
//Return the product of encoded1 and encoded2.
//
//Note: Compression should be done such that the run-length encoded array has the
...
//[6,6,6,3,3,3].
//prodNums = [6,6,6,6,6,6], which is compressed into the run-length encoded array
//[[6,6]].
//
//Example 2:
...
//[2,2,2,3,3,3].
//prodNums = [2,2,2,6,9,9], which is compressed into the run-length encoded array
//[[2,3],[6,1],[9,2]].
// 
//Constraints:
