...
//in the backlog. If that sell order's price is smaller than or equal to the
...
//buy order will be removed from the backlog. Else, the sell order is added to
//the backlog.
//Return the total amount of orders in the backlog after placing all the orders
//from the input. Since this number can be large, return it modulo 109 + 7.
// 
//
//Example 1:
...
//- 2 orders of type sell with price 15 are placed. There are no buy orders with
//prices larger than or equal to 15, so the 2 orders are added to the backlog.
...
//backlog.
//- 4 orders of type buy with price 30 are placed. The first 2 orders are matched
//with the 2 sell orders of the least price, which is 15 and these 2 sell orders
//are removed from the backlog. The 3rd order is matched with the sell order of
//the least price, which is 25 and this sell order is removed from the backlog.
//Then, there are no more sell orders in the backlog, so the 4th order is added
//to the backlog.
//Finally, the backlog has 5 buy orders with price 10, and 1 buy order with price
//30. So the total number of orders in the backlog is 6.
//
//Example 2:
//Input: orders = [[7,1000000000,1],[15,3,0],[5,999999995,0],[5,1,1]]
//Output: 999999984
//Explanation: Here is what happens with the orders:
//- 109 orders of type sell with price 7 are placed. There are no buy orders, so
//the 109 orders are added to the backlog.
...
//- 1 order of type sell with price 5 is placed. It is matched with the buy order
//of the highest price, which is 5, and that buy order is removed from the
//backlog.
