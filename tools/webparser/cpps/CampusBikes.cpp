...
//On a campus represented on the X-Y plane, there are n workers and m bikes, with
//n <= m.
//You are given an array workers of length n where workers[i] = [xi, yi] is the
//position of the ith worker. You are also given an array bikes of length m where
//bikes[j] = [xj, yj] is the position of the jth bike. All the given positions
//are unique.
//Assign a bike to each worker. Among the available bikes and workers, we choose
//the (workeri, bikej) pair with the shortest Manhattan distance between each
...
//Return an array answer of length n, where answer[i] is the index (0-indexed) of
//the bike that the ith worker is assigned to.
