...
//infinite supply). A package can be placed in a box if the size of the package
...
//total wasted space is minimized. For each package in a box, we define the space
//wasted to be size of the box - size of the package. The total wasted space is
//the sum of the space wasted in all the boxes.
//For example, if you have to fit packages with sizes [2,3,5] and the supplier
//offers boxes of sizes [4,8], you can fit the packages of size-2 and size-3 into
//two boxes of size-4 and the package with size-5 into a box of size-8. This
