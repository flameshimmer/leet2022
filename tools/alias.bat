DOSKEY t=textpad $*
DOSKEY s="C:\Program Files\Sublime Text 3\sublime_text.exe" $*
DOSKEY ..=cd ..
DOSKEY ...=cd ../..
DOSKEY ....=cd ../../..
DOSKEY dt=cd C:\Users\flame\Desktop
DOSKEY dl=cd C:\Users\flame\Download
DOSKEY code=cd C:\Users\flame\OneDrive\Desktop\2022\Leet2022
DOSKEY me=notepad C:\Users\flame\OneDrive\Desktop\2022\Leet2022\tools\alias.bat
DOSKEY reload=C:\Users\flame\OneDrive\Desktop\2022\Leet2022\tools\alias.bat
DOSKEY ls=dir $*
DOSKEY gs=git status
DOSKEY ga=git add *
DOSKEY gc=git commit -m $*
DOSKEY gp=git push
DOSKEY open=start $*
DOSKEY addall="git add *.cpp && git add *.h && git add *.pl"
DOSKEY commit=git commit -m  $*
alias fm="perl format.pl"
DOSKEY run=perl run.pl