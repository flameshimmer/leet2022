...
//There is a long and thin painting that can be represented by a number line. The
//painting was painted with multiple overlapping segments where each segment was
//painted with a unique color. You are given a 2D integer array segments, where
//segments[i] = [starti, endi, colori] represents the half-closed segment
...
//For example, if colors 2, 4, and 6 are mixed, then the resulting mixed color is
//{2,4,6}.
...
//by the 2D array painting where painting[j] = [leftj, rightj, mixj] describes a
//half-closed segment [leftj, rightj) with the mixed color sum of mixj.
...
//
//Note that returning a single segment [1,7) is incorrect because the mixed color
//sets are different.
// 
//Constraints:
