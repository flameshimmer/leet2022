...
//videoId starting from 0. Once a video is deleted, the videoId associated with
//that video can be reused for another video.
...
//return the substring of the video string starting at startMinute and ending at
...
//int[] getLikesAndDislikes(int videoId) Return a 0-indexed integer array values
//of length 2 where values[0] is the number of likes and values[1] is the number
//of dislikes on the video associated with videoId. If there is no video
...
//"getViews"]
//[[], ["123"], ["456"], [4], [0], ["789"], [1, 0, 5], [1, 0, 1], [1], [1], [1],
//[1], [1]]
//Output
//[null, 0, 1, null, null, 0, "456", "45", null, null, null, [1, 2], 2]
//Explanation
//VideoSharingPlatform videoSharingPlatform = new VideoSharingPlatform();
//videoSharingPlatform.upload("123");          // The smallest available videoId
//is 0, so return 0.
//videoSharingPlatform.upload("456");          // The smallest available videoId
//is 1, so return 1.
//videoSharingPlatform.remove(4);              // There is no video associated
//with videoId 4, so do nothing.
//videoSharingPlatform.remove(0);              // Remove the video associated
//with videoId 0.
//videoSharingPlatform.upload("789");          // Since the video associated with
//videoId 0 was deleted,
//                                             // 0 is the smallest available
//videoId, so return 0.
//videoSharingPlatform.watch(1, 0, 5);         // The video associated with
//videoId 1 is "456".
//                                             // The video from minute 0 to
//min(5, 3 - 1) = 2 is "456", so return "453".
//videoSharingPlatform.watch(1, 0, 1);         // The video associated with
//videoId 1 is "456".
//                                             // The video from minute 0 to
//min(1, 3 - 1) = 1 is "45", so return "45".
//videoSharingPlatform.like(1);                // Increase the number of likes on
//the video associated with videoId 1.
//videoSharingPlatform.dislike(1);             // Increase the number of dislikes
//on the video associated with videoId 1.
//videoSharingPlatform.dislike(1);             // Increase the number of dislikes
//on the video associated with videoId 1.
//videoSharingPlatform.getLikesAndDislikes(1); // There is 1 like and 2 dislikes
//on the video associated with videoId 1, so return [1, 2].
//videoSharingPlatform.getViews(1);            // The video associated with
//videoId 1 has 2 views, so return 2.
//
//Example 2:
//Input
...
//Explanation
//VideoSharingPlatform videoSharingPlatform = new VideoSharingPlatform();
//videoSharingPlatform.remove(0);              // There is no video associated
//with videoId 0, so do nothing.
//videoSharingPlatform.watch(0, 0, 1);         // There is no video associated
//with videoId 0, so return "-1".
//videoSharingPlatform.like(0);                // There is no video associated
//with videoId 0, so do nothing.
//videoSharingPlatform.dislike(0);             // There is no video associated
//with videoId 0, so do nothing.
//videoSharingPlatform.getLikesAndDislikes(0); // There is no video associated
//with videoId 0, so return [-1].
//videoSharingPlatform.getViews(0);            // There is no video associated
//with videoId 0, so return -1.
// 
//Constraints:
//1 <= video.length <= 105
