...
//    else if pivot < target, remove pivot and all elements to its left from the
//sequence
//    else, remove pivot and all elements to its right from the sequence
//  end while
//  return false
//When the sequence is sorted, the function works correctly for all values. When
//the sequence is not sorted, the function does not work for all values, but may
//still work for some values.
...
//Since the sequence has only one element, 7 will be chosen as the pivot. Because
//the pivot equals the target, the function will return true.
//
//Example 2:
//Input: nums = [-1,5,2]
//Output: 1
...
//If 5 was chosen as the pivot, 5 and 2 would be removed. In the next loop, the
//sequence would have only -1 and the function would return true.
...
//If 2 was chosen as the pivot, -1, 5 and 2 would be removed. The sequence would
//be empty and the function would return false.
//Searching for value 2 is NOT guaranteed to be found.
//If 5 was chosen as the pivot, 5 and 2 would be removed. In the next loop, the
//sequence would have only -1 and the function would return false.
