...
namespace Solution2022
{
	namespace PathSumII
	{
		...
		void helper(TreeNode* node, int targetSum, vector<int>& result, vector<vector<int>>& results) {
			if (!node) { return; }
			...
			result.push_back(node->val);
			helper(node->left, remain, result, results);
			helper(node->right, remain, result, results);
			result.pop_back();
		}
