...
//int join(int[] ownedChunks): A new user joined the system owning some chunks of
//the file, the system should assign an id to the user which is the smallest
//positive integer not taken by any other user. Return the assigned id.
//void leave(int userID): The user with userID will leave the system, you cannot
//take file chunks from them anymore.
...
//file of 4 chunks.
//fileSharing.join([1, 2]);    // A user who has chunks [1,2] joined the system,
//assign id = 1 to them and return 1.
//fileSharing.join([2, 3]);    // A user who has chunks [2,3] joined the system,
...
//chunk, as only the user with id = 2 has the file, return [2] . Notice that user
//1 now has chunks [1,2,3].
//fileSharing.request(2, 2);   // The user with id = 2 requested the second file
//chunk, users with ids [1,2] have this chunk, thus we return [1,2].
...
//their unique ID and users disconnect and connect from the system with the same
//IP?
...
//If all users join the system one time, request all files, and then leave, will
//your solution still be efficient?
